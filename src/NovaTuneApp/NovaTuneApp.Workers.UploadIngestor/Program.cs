...
using NovaTuneApp.Workers.UploadIngestor.Handlers;
...
try
{
    ...
    builder.Services.AddKafka(kafka => kafka
        ...
        .AddCluster(cluster =>
        {
            ...
            // MinIO events consumer
            cluster.AddConsumer(consumer => consumer
                .Topic($"{topicPrefix}-minio-events")
                .WithGroupId($"{topicPrefix}-upload-ingestor")
                .WithBufferSize(100)
                .WithWorkersCount(3)
                ...
                .AddMiddlewares(m => m
                    ...
                    .AddTypedHandlers(h => h.AddHandler<MinioEventHandler>())
                )
            );
        })
    );
    ...
    // Services
    // ============================================================================
    builder.Services.AddTransient<MinioEventHandler>();
    builder.Services.AddScoped<IUploadIngestorService, UploadIngestorService>();
...
internal class KafkaFlowHostedService : BackgroundService
{
    ...
    {
        _logger.LogInformation("Starting Upload Ingestor KafkaFlow bus...");
