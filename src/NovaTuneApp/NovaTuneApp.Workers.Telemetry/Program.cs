...
using NovaTuneApp.Workers.Telemetry.Handlers;
...
try
{
    ...
    builder.Services.Configure<TelemetryWorkerOptions>(
        builder.Configuration.GetSection(TelemetryWorkerOptions.SectionName));
    ...
    var workerOptions = builder.Configuration
        .GetSection(TelemetryWorkerOptions.SectionName)
        .Get<TelemetryWorkerOptions>() ?? new TelemetryWorkerOptions();
    ...
    builder.Services.AddHealthChecks()
        ...
        .AddKafka(
            ...
            name: "kafka",
            timeout: TimeSpan.FromSeconds(5))
        .AddCheck<TelemetryWorkerHealthCheck>("telemetry-worker");
    ...
    builder.Services.AddKafka(kafka => kafka
        ...
        .AddCluster(cluster =>
        {
            ...
            // Telemetry events consumer
            cluster.AddConsumer(consumer => consumer
                .Topic($"{topicPrefix}-telemetry-events")
                .WithGroupId($"{topicPrefix}-telemetry-worker")
                .WithBufferSize(workerOptions.ConsumerBufferSize)
                .WithWorkersCount(workerOptions.ConsumerWorkerCount)
                ...
                .AddMiddlewares(m => m
                    ...
                    .AddTypedHandlers(h => h.AddHandler<TelemetryEventHandler>())
                )
            );
        })
    );
    ...
    // Services
    // ============================================================================
    builder.Services.AddTransient<TelemetryEventHandler>();
...
{
    Log.Fatal(ex, "Telemetry worker terminated unexpectedly");
}
finally
{
    Log.CloseAndFlush();
...
internal class KafkaFlowHostedService : BackgroundService
{
    ...
    {
        _logger.LogInformation("Starting Telemetry Worker KafkaFlow bus...");
