...
## UI Test Fixing (Post-Run)

When assigned the **Post-run** phase of `tasks/add_ui_tests/main_exec.md`, debug and fix flaky or failing Selenium UI tests.

### Key Files
- **UI test project**: `src/ui_tests/NovaTuneApp.UiTests/`
- **Fixtures**: `Fixtures/UiTestFixture.cs`, `Fixtures/WebDriverFactory.cs`
- **Helpers**: `Infrastructure/WaitHelpers.cs`, `Infrastructure/ScreenshotHelper.cs`, `Infrastructure/UiTestBase.cs`
- **Failure screenshots**: `src/ui_tests/NovaTuneApp.UiTests/screenshots/`

### Debug Strategy
1. Check failure screenshots in `screenshots/` directory
2. Run headed: `UI_TESTS_HEADED=1 dotnet test --filter "FullyQualifiedName~FailingTest"`
3. Increase timeout in `WaitHelpers` calls if elements load slowly
4. Verify `data-testid` attributes exist in Vue components: `grep -n "data-testid" <file>`
5. Check Vite dev server health (ports 26173/26174)
6. Never add `Thread.Sleep` — only adjust `WebDriverWait` timeouts

### Validation
```bash
dotnet test src/ui_tests/NovaTuneApp.UiTests/NovaTuneApp.UiTests.csproj -c Debug -v detailed
```
