...
## UI Test Support (Phase 0)

When assigned **Phase 0** of `tasks/add_ui_tests/main_exec.md`, add `data-testid` attributes to Vue components for Selenium test selectors. Follow the conventions in `.claude/skills/add-data-testid/SKILL.md`. Key rules:
- Attribute goes on the **opening tag**: `<div ... data-testid="foo">`
- Form inputs: match `v-model` name (`email`, `password`, `displayName`, `confirmPassword`)
- Buttons: `{action}-button` (`login-button`, `register-button`, `create-playlist-button`)
- Error/success divs: `error-message`, `success-message`
- Headings: `{page}-heading` (`library-heading`, `dashboard-heading`)
- Empty states: `empty-state`
- Lists: `{item}-list` (`track-list`)

After adding, verify: `cd src/NovaTuneClient && pnpm lint && pnpm typecheck`
